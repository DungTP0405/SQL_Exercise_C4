...
--3.6 Same as previous exercise, but select only those warehouses where the average 
...
--3.8 Select the warehouse codes, along with the number of boxes in each warehouse. 
    -- Optionally, take into account that some warehouses are empty (i.e., the box count 
    -- should show up as zero, instead of omitting the warehouse from the result).
SELECT w.code, Num_boxes
FROM (
    SELECT Warehouse, COUNT(code) AS Num_boxes
    FROM Boxes
    GROUP BY Warehouse) AS A
RIGHT JOIN Warehouses w
ON A.warehouse = w.code

--3.9 Select the codes of all warehouses that are saturated (a warehouse is saturated if 
...
SELECT *, Value*0.85 AS [Reduced value]
FROM Boxes
...
DROP INDEX Boxes.idx_warehouse